    ...
    public partial class GestCourses : Form
    {
        ...
        private void buttonModifier_Click(object sender, EventArgs e)
        {
            if (IdxLigneActuelle != -1)
            {
                ...
                if (reponse == DialogResult.Yes)
                {
                    ...
                    Courses creeCourse = new Courses(textBoxNom.Text, Convert.ToDateTime(textBoxDate.Text), textBoxDistance.Text, Convert.ToDateTime(textBoxHeureDep.Text), Convert.ToInt16(textBoxEnCours.Text));
        ...
        private void buttonSupprimer_Click(object sender, EventArgs e)
        {
            if (IdxLigneActuelle != -1)
            {
                ...
                if (reponse == DialogResult.Yes)
                {
                    Courses creeCourse = new Courses(ligneSelect.Cells[1].Value.ToString(), Convert.ToDateTime(ligneSelect.Cells[2].Value.ToString()), ligneSelect.Cells[3].Value.ToString(), Convert.ToDateTime(ligneSelect.Cells[4].Value.ToString()), Convert.ToInt16(ligneSelect.Cells[5].Value.ToString()));
        ...
        private void dataGridViewClubs_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            textBoxEnCours.Text = ligneSelect.Cells[5].Value.ToString();
        }
            ...
            if (reader != null)          // on teste si la requete a bien retournéer un résultat
            {
                ...
                if (reader.HasRows)
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        dataGridViewCourses.Rows.Insert(i,Convert.ToInt16( reader.GetString(0)), reader.GetString(1), reader.GetString(2), reader.GetString(3), reader.GetString(4), reader.GetString(5));
                        i ++;
                    }
